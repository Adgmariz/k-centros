...
#class_to_id = {'BRICKFACE': 0, 'SKY': 1, 'FOLIAGE': 2, 'CEMENT': 3, 'WINDOW': 4, 'PATH': 5, 'GRASS': 6}
#col_names = ['class'] + [str(i) for i in range(numbers_per_line - 1)] #inverse order
#df['class'] = df['class'].map(class_to_id)
filepath = 'datasets/image+segmentation/segmentation.test'
output_file = 'segmentation.csv'
k = 7
numbers_per_line = 20 #including the class
...
col_names = ['class'] + [str(i) for i in range(numbers_per_line - 1)]
class_to_id = {
    'BRICKFACE': 0, 
    'SKY': 1, 
    'FOLIAGE': 2, 
